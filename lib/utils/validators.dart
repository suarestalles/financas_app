...
class Validators {
  ...
  static FormFieldValidator date(
    ...
    return (data) {
      if ((data as String).isEmpty) {
        return null;
      } else if (data.contains(RegExp(r'\d{2}\/\d{2}\/\d{4}'))) {
        try {
          //Explode a data
          var expDate = data.split(RegExp(r'(/|-)'));
  ...
  static FormFieldValidator validaDataSuperiorAtual(
    ...
    return (data) {
      if (data.contains(RegExp(r'\d{2}\/\d{2}\/\d{4}'))) {
        try {
          //Explode a data passada
          var expDateP = data.split(RegExp(r'(/|-)'));
          ...
          var expDateA = DateTime.now().toString().split(RegExp(r'(/|-)'));
